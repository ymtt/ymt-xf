...
function getfocus(){
    var session=window.sessionStorage.getItem("session");
    var url='http://101.204.236.5/fw?t=app&controller=com.xfsm.action.ExpertAction&m=myFcous&start=1&pagesize=';
    $.ajax({
        type:'get',
        dataType:'json',
        url:url,
        ...
        success:function(data){
            ...
            $.each(list,function(key){
                //专家头像
                var head_pic='http://101.204.236.5/'+list[key]['expert_head_pic']
                //专家名字
...
function CancelFocus(expert_id){
    ...
    var url='http://101.204.236.5/fw?controller=com.xfsm.action.PersonalAction&method=cancel';
    $.ajax({
        type:'get',
        dataType:'json',
        url:url,
